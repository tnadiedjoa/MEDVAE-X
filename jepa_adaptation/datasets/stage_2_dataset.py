...
def build_stage_2_dataset(
    ...
    for name in MEDMNIST_2D_DATASETS:
        ...
        try:
            ...
            print(f"[stage_2_dataset] skip {name}/{split}: {e}")

    if not datasets:
        raise RuntimeError("No MedMNIST stage-2 datasets could be loaded.")

    return ConcatDataset(datasets)


if __name__ == "__main__":
    ds = build_stage_2_dataset(split="train", size=64, download=True, fraction=0.1)
    print(f"Stage-2 train samples : {len(ds)}")
