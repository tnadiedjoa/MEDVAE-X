...
\node[title] at (5.0,16.9) {Ablation par signal $c$ constant};

\node[data] (data) at (5.0,15.2) {ARCADE train, 1000 images\\m\^eme architecture};

\node[model]      (creal)  at (2.6,12.9) {cVAE($c$ r\'eel)\\warmup+FT, 7500 steps};
\node[constmodel] (cconst) at (7.4,12.9) {cVAE($c{=}0{,}5$)\\m\^eme budget, m\^eme arch.};
...
\node[font=\sffamily\scriptsize\itshape,text=black!60,align=center] at (5.0,9.5)
  {seule variable chang\'ee : la valeur de $c$\\vue \`a l'entra\^inement};

\node[data] (testset) at (5.0,8.0) {Test (60 images)\\inf\'erence avec le vrai $c$};
...
\node[metricbox,fill=loss1] (metrics) at (5.0,5.8)
  {MSE, SSIM, PSNR, HaarPSI\\test de Wilcoxon};

\draw[arrow] (testset.south) -- (metrics.north);

\node[merge] (concl) at (5.0,3.1)
  {Signal $c$ exploit\'e\\pour l'approche C\\{\scriptsize\mdseries (4/4 m\'etriques sig.,}\\{\scriptsize\mdseries effet faible/invers\'e pour A/B)}};
