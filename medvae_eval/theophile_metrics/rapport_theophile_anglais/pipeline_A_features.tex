...
\node[title] at (4.5,14.2) {Construction of quality features};

\node[data] (img) at (4.5,12.8) {ARCADE image\\$512\times512$, $\times1200$};

\node[model]  (mscn)   at (2.4,10.6) {MSCN + 8 metrics/patch\\$4\times4$ grid};
\node[resnet] (resnet) at (6.6,10.6) {ResNet-18\\ImageNet, frozen};

\draw[arrow] (img.south) -- (4.5,11.7) -- (2.4,11.7) -- (mscn.north);
\draw[arrow] (img.south) -- (4.5,11.7) -- (6.6,11.7) -- (resnet.north);

\node[model]  (agg)      at (2.4,8.4) {Aggregation\\mean/std/max/min};
...
\node[font=\sffamily\scriptsize\itshape,text=black!60,align=center] at (6.6,6.2)
  {used only by\\the learned\\score approach};
