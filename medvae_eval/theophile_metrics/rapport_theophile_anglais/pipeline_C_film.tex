...
\node[title] at (16.3,11.0) {FiLM conditioning and training};

\node[data] (c) at (2.0,8.6) {$c \in [0,1]$};
\node[film] (embed) at (7.0,8.6) {Shared MLP\\$1\to128\to128$};
\node[film] (heads) at (12.6,8.6) {19 linear heads\\$(\gamma_i,\beta_i)$, zero init.};
\node[vae] (resblocks) at (19.0,8.6) {19 ResnetBlocks\\Enc.(8)+bottleneck(2)+Dec.(11)\\$h\leftarrow(1{+}\gamma_i)h+\beta_i$};
\node[ckpt,draw=medBlue,fill=medBlue!15] (cvaeckpt) at (25.6,8.6) {cVAE(real $c$)\\val\_loss $=0.00925$};
...
\node[phasebox,fill=phase1] (p1) at (5.0,5.4) {FiLM warm-up\\2500 steps, $\eta{=}4.5{\cdot}10^{-6}$};
\node[phasebox,fill=phase2] (p2) at (12.2,5.4) {Global fine-tuning\\5000 steps, $\eta{=}10^{-6}{\to}10^{-7}$};
...
\node[baserow] (basevae) at (5.0,2.0) {Baseline VAE\\identical, no FiLM};
\node[phasebox,fill=baseGray!50] (pbase) at (12.2,2.0) {Identical training\\7500 steps, same hyperparams};
...
\node[font=\sffamily\Large\itshape,text=black!60,align=center] at (5.0,0.4)
  {ablation: same budget, same data, no conditioning};
