"""
MedVAE Standalone — AutoencoderKL 2D (architecture identique au repo officiel StanfordMIMI/MedVAE).

Ce fichier regroupe toutes les briques nécessaires pour utiliser l'AutoencoderKL 2D
sans dépendre du dossier jepa-adaptation/ ni d'imports externes complexes.

Source : https://github.com/StanfordMIMI/MedVAE
Licence : MIT
...
# ═══════════════════════════════════════════════════════════════
#  Helper functions & modules (diffusion model building blocks)
# ═══════════════════════════════════════════════════════════════
...
class LinearAttention(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        qkv = self.to_qkv(x)
        # Reshape without einops: (b, 3*heads*dim_head, h, w) → 3 x (b, heads, dim_head, h*w)
        dim_head = qkv.shape[1] // (3 * self.heads)
        qkv = qkv.reshape(b, 3, self.heads, dim_head, h * w)
        q, k, v = qkv[:, 0], qkv[:, 1], qkv[:, 2]  # each (b, heads, dim_head, hw)

        k = k.softmax(dim=-1)
        context = torch.einsum("bhdn,bhen->bhde", k, v)
        out = torch.einsum("bhde,bhdn->bhen", context, q)
        out = out.reshape(b, self.heads * dim_head, h, w)
...
class Decoder(nn.Module):
    def __init__(
        self, *, ch, out_ch, ch_mult=(1, 2, 4, 8), num_res_blocks,
        attn_resolutions, dropout=0.0, resamp_with_conv=True,
        in_channels, resolution, z_channels, give_pre_end=False,
...
# ═══════════════════════════════════════════════════════════════
#  AutoencoderKL — VAE 2D complet
# ═══════════════════════════════════════════════════════════════

class AutoencoderKL(nn.Module):
    """
    AutoencoderKL 2D — architecture identique au repo officiel MedVAE.

    Inclut le channel_ds (skip-connected 1×1→3×3→1×1 conv) et le channel_proj
    utilisés pour produire la représentation latente « projetée ».
    """

    def __init__(
        self, ddconfig, embed_dim, ckpt_path=None,
        ignore_keys=[], apply_channel_ds=True, state_dict=True,
    ):
        super().__init__()
    ...
    def init_from_ckpt(self, path, ignore_keys=None, state_dict=True):
        ...
        print(
            f"Restored from {path} with {len(missing)} missing "
